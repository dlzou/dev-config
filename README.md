...
The script installs missing Neovim, basedpyright, Ruff, ripgrep, fd, and tree-sitter CLI.
...
from Xcode Command Line Tools; curl and tar come with macOS. Rerunning the script
...
tree-sitter CLI, Git, Make, GCC, curl, and archive utilities. The `path:` form also
includes new configuration files before they have been added to Git. Nothing is
...
- `:checkhealth userconfig` checks external tools and minimum versions; also use
  `:checkhealth lazy`, `:checkhealth nvim-treesitter`, `:checkhealth vim.lsp`, and
...
| Space fb / ff / fg / ft | Buffers / files / text search / file tree |
